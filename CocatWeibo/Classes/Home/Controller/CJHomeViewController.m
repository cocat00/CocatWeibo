...
@property (nonatomic, weak) CJTitleButton *titleButton;
...
{
    // 1.封装请求参数
    CJUserInfoParam *param = [CJUserInfoParam param];
    param.uid = @([CJAccountTool account].uid);
    
    // 2.发送请求
    [CJUserTool userInfoWithParam:param success:^(CJUserInfoResult *result) {
        // 设置标题文字
...
{
    CJHomeStatusesParam *param =[CJHomeStatusesParam param];
    param.count = @(20);
    ...
    // 刷新数据(向新浪获取更新的微博数据)
    [CJStatusTool homeStatusesWithParam:param success:^(CJHomeStatusesResult *result) {
        // 取出所有的微博数据，用MJExtennsion
//        NSArray *statusArray = [CJStatus mj_objectArrayWithKeyValuesArray:json[@"statuses"]];
...
- (void)findFriend:(UIButton *)button
{
    NSLog(@"findFriend");
}
...
- (void)titleClick:(UIButton *)button
{
    ...
    if ([buttonImage isEqual: [UIImage imageWithName:@"navigationbar_arrow_up"]])
    {
